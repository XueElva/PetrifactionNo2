...
public class ActivityLianxiren extends Activity {
	XListView mListView;
	Myadapter mAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
					long arg3) {
				try {
					lianxirenStr = jsarrdata.getJSONObject(arg2 - 1).toString();
					finish();
	...
	public void finish() {
		final EditText name = (EditText) findViewById(R.id.qiyemingcheng);
		if (lianxirenStr.length() > 0) {
			Intent intent = new Intent();
			intent.putExtra("lianxiren", lianxirenStr);
			setResult(40, intent);
	...
	public void getNews(final String str) {
		...
		params.put("q", str);
		params.put("accessToken", UserInfoTools.getUserSH(ActivityLianxiren.this)
		...
		AsyncHttpResponseHandler hd = new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, String content) {
				...
				try {
					jsarrdata = new JSONObject(content).getJSONObject("data").getJSONArray("message");
		...
		client.get(MyConfig.IP + "sd/lxr.do", params, hd);
	}
}